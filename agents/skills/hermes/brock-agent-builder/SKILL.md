...
- **Lead dashboards must match the Accor Plus depth standard.** When building or reviewing a `crew-web-lead-dashboard-builder` skill, the bar is the Accor Plus Outreach Dashboard (https://accor-plus-leads-dashboard.vercel.app/). A passing dashboard must include: fit scoring (0-100, hot leads flagged at 90+), LinkedIn research as the default path with named decision-makers, derived emails with "verify before send" warnings, personalised insights per lead that name the specific mechanism not the category, LinkedIn DMs drafted alongside cold emails, filter controls (region, quality), a metrics row, staff size estimates, and a calendar question after the build. Thin cards with "LinkedIn research not enabled" on every row fail the standard. See `references/lead-dashboard-depth-standard.md` for the full specification.
