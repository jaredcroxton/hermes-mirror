#!/usr/bin/env python3
"""
generate_assets.py - APOGEE cinematic descent asset pipeline (kie.ai REST, stdlib only).

Two stages:
  1. keyframes : nano-banana generates 4 art-directed stills (A space, B cloud-top,
                 C below-cloud tower, D penthouse interior). The hosted result URLs
                 are reused directly as seedance first/last frame anchors.
  2. clips     : seedance-2 image-to-video. Each clip is anchored first=keyframe[i],
                 last=keyframe[i+1], so clip N ends exactly where clip N+1 begins.
                 Shared boundary stills = one continuous, seamless descent.

Verified KIE contract (see aether-genesis/tools/kie_video.py):
...
Usage:
  ...
  python3 pipeline/generate_assets.py --keyframes    # generate A..D, cache hosted urls
  python3 pipeline/generate_assets.py --clips        # generate clip1..3 (needs keyframe urls)
  python3 pipeline/generate_assets.py --all          # keyframes then clips
"""
...
def do_clips(key):
    ...
    for clip in cfg["clips"]:
        ...
        print("[%s] %s  seed=%s" % (clip["id"], cfg["model"], clip["first"]), flush=True)
        tid = create_task(cfg["model"], {
            "prompt": clip["prompt"],
            "image_url": seed,
        ...
        url = poll(tid, key)
        download(url, os.path.join(ASSETS, "video", clip["id"] + ".mp4"))
