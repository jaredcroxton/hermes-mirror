...
**Note:** One project per client carries everything, but only the first two steps feed each other: the research feeds the outreach. The quote starts from the terms you agreed on the call, and the invoice workflow designs how invoicing runs at all, so neither reads the step above it. They stay in the one project so the client's whole story lives in one place.
...
1. `crew-finance-expense-review`
2. `crew-finance-monthly-summary`
3. `crew-finance-cashflow-brief`
4. `crew-finance-invoice-workflow`

**Note:** One sitting each month: what you spent, what happened, where cash pinches, who to chase. Expenses lead because the month-end summary pulls the grouped expense detail and exceptions before it summarises; run it the other way round and the summary is written blind.
...
1. `crew-marketing-content-repurpose`
2. `crew-marketing-social-post-pack`
3. `crew-marketing-email-campaign-builder`
4. `crew-marketing-brand-voice-check`

**Note:** One good piece becomes a fortnight of content across every channel, voice-checked before any of it goes out. The voice check runs last because its first required input is the draft copy: at the front of the chain there is nothing for it to read.
