...
For Zapier MCP, Hermes can use mcporter as a stdio bridge:

```yaml
mcp_servers:
  zapier:
    command: "npx"
    args: ["-y", "mcporter", "serve", "--servers", "zapier", "--stdio"]
    timeout: 120
    connect_timeout: 60
```
...
After `hermes mcp test zapier` discovers tools, call Zapier's onboarding skill through mcporter (`zapier.list_zapier_skills()` then `zapier.get_zapier_skill({"name":"zapier-mcp-onboarding"})`) and follow it step-by-step.
