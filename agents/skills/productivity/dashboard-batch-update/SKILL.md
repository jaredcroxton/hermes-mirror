...
- **Firecrawl may run out of credits.** Always be aware of the browser-based DOM scraping fallback. The `browser_navigate` + `browser_console` with JavaScript extraction pattern works for GitHub trending when Firecrawl is unavailable. If the user specifically asked to use Firecrawl but credits are exhausted, record that Firecrawl was attempted and name the fallback used in the batch JSON `scrapeNotes` rather than failing the dashboard run.
