...
**Superpowers is methodology, not a skill pack.** Superpowers is not a separate set of 13 skills to count alongside the Crew packs. It is the foundation — the 8 standards (Brainstorm, Plan, TDD, Debug, Verify, Review, Finish, Context) that every skill inherits. When counting the total skill architecture, count flow skills + crew packs. Do not count Superpowers as a separate skill pack. It is the bedrock underneath everything.

**Client skill pack email delivery:** When sending Crew skill packs to a client who does not use terminal, use the dead-simple install pattern in `references/crew-skill-pack-client-delivery.md`. The client unzips to Desktop, pastes two Claude Code copy commands, restarts Claude Code, and types a slash command. No terminal. No Finder. No hidden folders. Captured 17 June 2026.
