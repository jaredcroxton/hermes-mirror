...
1. **Research** — Outlier methodology: scrape top competitors, extract section structure, identify frequency patterns, find blue-ocean positioning gaps. For content strategy work involving external tools/platforms (AI video generation, SEO tools, etc.), deep-research the tool across 10+ sources first — official docs, community best practices, pricing pages, tutorial videos, prompt engineering guides. Become an expert before writing the strategy.
...
4. **Structure** — Write the full strategy as markdown first, then produce the operational Excel workbook. When Jared asks for a plan with sequence, deadlines, or trackable items, deliver BOTH: the markdown strategy document (for reading) AND the Excel workbook (for working). See `references/excel-operational-plan-delivery.md` for the Excel pattern, brand rules, phase colour coding, and openpyxl patterns.
...
- **Markdown-only delivery for operational plans.** When Jared asks for a plan with sequence, deadlines, or trackable items, delivering only markdown is incomplete. He expects an Excel workbook he can track, update, and work from. See `references/excel-operational-plan-delivery.md`. The markdown is the thinking; the Excel is the plan.
...
- **Generating content for unfamiliar tools without research (locked 18 June 2026).** When Jared asks for content, prompts, or assets built for a specific tool or model (e.g. Seedance 2.0, a new AI model, a platform he is testing), do not write from assumptions about what the tool can do. Research the tool's actual capabilities, limitations, and best practices FIRST. Then build. Writing 150-word narrative prompts for a model that only responds to 40-80 word director-style instructions produces garbage and wastes Jared's time. Jared's words: "those prompts are terrible. I want you to go away, do some extensive research on the model, and come back with the best practices." Correct sequence: research → understand → build → deliver.
...
- **Public copy must use buyer language, not internal terminology (locked 29 June 2026).** Jared corrected Brock for using internal development terms in public-facing Eventbrite copy: \"don't worry about gold standard, because that doesn't make any sense to the person.\" Public copy must be written from the buyer's perspective using language they understand. \"94 gold-standard AI agents\" means nothing to a buyer. \"Full-stack apps that connect to databases\" does. \"Deployed to Vercel\" means nothing. \"Live on the web\" does. Strip all internal product terminology (gold standard, QA pass, Vercel, Supabase, packs, packs, Claude Code unless the skill is literally about Claude Code) from public-facing copy. Write what the buyer gets, not what the product is made of. Jared's words: \"We want them to leverage that we're going to build full stack apps that connect databases. You'll leave the course with fully built websites and full stack apps with database and logging connected, coded.\"

- **Research deliverables must filter out irrelevant competitor patterns (locked 29 June 2026).** When producing competitive research for a specific business model, do not include patterns from competitors that operate on different models (enterprise pricing tiers, early-bird discounts, corporate upsells, multi-week programmes) unless those patterns genuinely apply. Jared's correction: \"why have you put in all the early bird stuff and about corporate and all those event things?\" The research file should be a sharp tool for the specific build, not a dump of everything observed. Filter against the actual business model before delivering.
...
- `references/eventbrite-workshop-listing-copy-pattern.md` — Eventbrite workshop listing copy: summary format, description beats, buyer-language rules, what to strip, FAQ format, and Jared's specific corrections. Use when writing public-facing event copy for one-day build workshops.
