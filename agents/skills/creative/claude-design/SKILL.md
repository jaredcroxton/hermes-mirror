...
**Before starting, check `design-md` (Google's DESIGN.md token spec format).** If the deliverable is a token spec file rather than a rendered artifact, use `design-md` instead.

## Design System Library

When the user says "make it look like Stripe / Linear / Vercel / Notion / Apple" — load the matching design system from `references/design-systems/<site>.md`. 54 sites are available covering AI/ML, dev tools, infra, design tools, fintech, and enterprise brands. Each file captures a site's complete visual language: color palette, typography hierarchy, component styles, spacing system, shadows, and responsive behavior with exact CSS values and CDN font substitutes.

**Usage:**
```bash
skill_view(name="claude-design", file_path="references/design-systems/stripe.md")
```

**Design catalog:** `airbnb.md`, `airtable.md`, `apple.md`, `bmw.md`, `cal.md`, `claude.md`, `clay.md`, `clickhouse.md`, `cohere.md`, `coinbase.md`, `composio.md`, `cursor.md`, `elevenlabs.md`, `expo.md`, `figma.md`, `framer.md`, `hashicorp.md`, `ibm.md`, `intercom.md`, `kraken.md`, `linear.app.md`, `lovable.md`, `minimax.md`, `mintlify.md`, `miro.md`, `mistral.ai.md`, `mongodb.md`, `notion.md`, `nvidia.md`, `ollama.md`, `opencode.ai.md`, `pinterest.md`, `posthog.md`, `raycast.md`, `replicate.md`, `resend.md`, `revolut.md`, `runwayml.md`, `sanity.md`, `sentry.md`, `spacex.md`, `spotify.md`, `stripe.md`, `supabase.md`, `superhuman.md`, `together.ai.md`, `uber.md`, `vercel.md`, `voltagent.md`, `warp.md`, `webflow.md`, `wise.md`, `x.ai.md`, `zapier.md`
...
| Airbnb Cereal VF | DM Sans |
| Circular (Spotify) | DM Sans |
| figmaSans | Inter |
| IBM Plex Sans/Mono | IBM Plex Sans/Mono |

**Related:** For authoring formal DESIGN.md token spec files, use the `design-md` skill.
...
- **motion by default for premium HTML decks**: subtle slide transitions, staggered content entrances, hover/focus microinteractions, and an ambient layer such as particles, glow, drifting gradients, or grid/parallax when it fits the brief
- respect `prefers-reduced-motion` and provide a low/no-motion path; motion must never cause layout overflow or delay comprehension
...
- stays subtle
- for executive slide decks, adds keynote polish through staged entrances, soft cross-slide movement, slow ambient depth layers, progress/nav feedback, and tasteful hover states
