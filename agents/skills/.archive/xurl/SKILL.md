...
### Social listening / “scrape X” requests

When the user asks to scrape, search, monitor, or gather information from X/Twitter, use the official API via `xurl` rather than browser scraping. Treat this as a read-only research workflow unless the user explicitly asks for a write action.

1. Verify setup without exposing secrets:
   ```bash
   command -v xurl
   xurl auth status
   ```
   If `xurl` is missing, give the install command from the Installation section. If auth is missing, direct the user to the One-Time User Setup section; never inspect `~/.xurl` or ask for pasted tokens.
2. Start with a small reachability query before doing larger collection:
   ```bash
   xurl search "QUERY" -n 3
   ```
3. For market/social-listening tasks, run several focused queries instead of one broad query. Combine brand terms, competitor terms, customer-problem terms, and geography/language filters where supported by X search. Examples:
   ```bash
   xurl search '"Accor Plus" Australia hotel membership lang:en' -n 20
   xurl search '"Accor Plus" OR "ALL Accor" hotel deals Australia lang:en' -n 20
   xurl search 'Australian travel hotel deals dining discounts lang:en' -n 20
   ```
4. Summarize themes, representative posts, objections, sales angles, and caveats. Keep raw post IDs/URLs available when the user needs traceability.
5. Confirm intent before any write or engagement action: post, reply, like, repost, bookmark, follow, block, mute, delete, or DM.
